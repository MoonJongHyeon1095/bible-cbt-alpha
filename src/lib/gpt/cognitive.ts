// // src/lib/gpt/cognitive.ts// src/lib/gpt/cognitive.ts
...
//   const systemPrompt =
//     `당신은 인지행동치료 전문가입니다. 사용자가 작성한 경험과 자동적 사고를 깊이 분석하여, ` +
//     `그 안에 담긴 인지오류를 찾아내고 사용자의 원본 글을 구체적으로 인용하면서 왜 그런 오류가 나타났는지 설명해주세요.`;
...
// 위 내용을 바탕으로 다음 10가지 인지오류 중 해당하는 것을 최소 3개 선택하고,
// 각 오류마다 사용자가 쓴 글에서 구체적인 부분을 인용하면서 분석해주세요:

// ${COGNITIVE_ERRORS.map((e, i) => `${i + 1}. ${e.title}: ${e.description}`).join("\n")}
...
//     console.log("Cognitive Error Analysis Response:", response);

//     // 응답 파싱
//     const errorBlocks = response.split("---").filter((b) => b.trim());
//     const selectedErrors: CognitiveErrorAnalysisResult["errors"] = [];

//     for (const block of errorBlocks) {
//       const numberMatch = block.match(/오류번호[:\s]*(\d+)/);
//       const quoteMatch = block.match(/인용[:\s]*([\s\S]*?)(?=\n분석|$)/);
//       const analysisMatch = block.match(/분석[:\s]*([\s\S]*?)$/);

//       if (!numberMatch || !quoteMatch || !analysisMatch) continue;

//       const index = parseInt(numberMatch[1], 10) - 1;
//       if (Number.isNaN(index) || index < 0 || index >= COGNITIVE_ERRORS.length)
//         continue;

//       selectedErrors.push({
//         ...COGNITIVE_ERRORS[index],
...
//     return { errors: selectedErrors.slice(0, 5) };
//   } catch (error) {
