...
import { MessageSquare, Heart, Plus, AlertCircle } from 'lucide-react';
    ...
    <div className="max-w-[1000px] mx-auto px-8 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">비슷한 경험을 나누고 공감을 나눠보세요.</p>
        </div>
        {!isCreating && (
          <Button
      ...
      {isCreating && (
        <Card className="p-6 mb-6 bg-green-50 border-2 border-green-200">
          <h3 className="text-lg text-slate-900 mb-4">익명으로 경험 공유하기</h3>
          <div className="space-y-4">
            <div>
              <label className="text-sm text-slate-700 mb-2 block">카테고리</label>
      ...
      {posts.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-slate-500 text-lg mb-2">아직 게시된 글이 없습니다.</p>
          <p className="text-slate-400">첫 번째 글을 작성해보세요!</p>
        </Card>
      ) : (
        <div className="space-y-4">
          {posts.map((post) => (
            <Card key={post.id} className="p-5 hover:shadow-md transition-shadow bg-white">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-400">{formatDate(post.timestamp)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 pt-3 border-t border-slate-100">
                ...
                >
                  <Heart
                    className={`size-5 ${myEmpathies.has(post.id) ? 'fill-current' : ''}`}
                  />
                  <span className="text-sm">
                    {post.empathyCount > 0 ? `공감 ${post.empathyCount}` : '공감하기'}
                  </span>
                </button>
              </div>
            </Card>
